...
Last updated: 2026-08-07 · data.js at DATA_VERSION 147
...
- **Split the monolith into consistent standalone pages (option 3).** `wardrobe_v2_18.html` is the last SPA (4 in-page tabs: Outfits / Wardrobe / Builder / History); every other destination is already standalone. Decisions locked (2026-08-05): land on **Today**; adopt clean filenames (`outfits.html` / `wardrobe.html` / `builder.html` / `history.html`, retire the `wardrobe_v2_18` name); phase-by-phase with a commit + test between each. Plan:
  ...
  - [x] **Phase 2 — peel tabs into standalone pages, least-coupled first (DONE 2026-08-07).**
    ...
    - [x] **Builder → builder.html (DONE 2026-08-07).** Standalone Suggestions (pieces + outfits via `pairsViewHtml`) + Closet (2-col fit-to-height, bonus-stat info, `CUSTOM_PROPOSALS` save). Reads items, uses `buildPairings`.
  ...
  - [x] **outfits.html popup migration + CSS sweep (DONE 2026-08-07).** Board popups migrated onto modals.js (`Modals.openItem/openOutfit/openWear/openPhoto`, retired the `lb-*` lightbox). Dead-CSS sweep: 1303 → 1014 lines (~290 lines of lb-/builder/closet/mgr/picker/preview/vibe CSS gone incl. mobile `@media` + compound-selector stragglers); braces balanced, zero dead classes left, all live board classes intact, combined JS parses.
  - [~] **Shared `pieceCardHtml` (2026-08-05).** New `pieceCardHtml(piece, {view})` in data.js + `.piece-card` CSS in alter-theme (whole card is a `.piece-name-link`, gold `--accent2` hover matching `.mgr-item`/`.outfit-card`). Views: `chip` (compact horizontal) · `row` (thumb + role + name + brand). Migrated: the suggestion chips (`pairingPieceChip` → chip) and the wear-detail piece rows (`Modals.openWear` → row). Remaining/optional: the outfit-card grid `piece-row` + editorial `h-piece-card` already use shared classes + gold hover + `.piece-name-link`, so they're a different context (piece *lists* inside outfit cards) — fold into `pieceCardHtml` only if a fully single source is wanted.
...
- [x] **Finish CSS single-sourcing (moot 2026-08-07).** The monolith (`wardrobe_v2_18.html`) that held the duplicate inline copies is retired; its popup CSS was swept and popups now render through modals.js + the shared stylesheets. JS builders were already fully shared.
...
- [~] **Wear-log experiential layer (occasion · feeling · reactions).** `occasion` STARTED (2026-08-07): added an optional `occasion` field to wear entries (work / night-out / date / casual / party / event), tagged the first two (8/4 + 8/6, both "work"), and surfaced it as a chip in the wear-detail popup (`Modals.openWear`). REMAINING: (a) capture UI to set occasion when logging a wear; (b) backfill the older 8 entries; (c) aggregate on read — outfit popup "Worn to: Work ×2", item popup across looks; (d) filter History by occasion; (e) let Today bias suggestions toward outfits historically worn to the picked context; (f) then add `feelings` (powerful/confident/sexy/sharp/comfortable/playful, multi-tag) + `reactions` (free note + "got compliments"). Subjective layer complementing the objective stats.
...
- [~] **Localize product images** (highest priority). `tools/localize-images.py` (built 2026-08-07) — parses `data.js`, downloads each hotlinked `img` (+ any `images:[]` extras) into `item-photos/{id}.{ext}`, verifies the bytes are really an image (not an HTML error page), rewrites the paths in `data.js` (with a `.bak`), and leaves failed downloads hotlinked + reported. Node-free, macOS python3, **run locally** (needs egress). Link-check found **19 hotlinked heroes, 18 reachable** — only yg1's GOAT image is a real 404 (needs a replacement sourced). DONE 2026-08-07: ran locally — **all 19 heroes downloaded (0 failed)**, `data.js` rewritten to `item-photos/{id}.{ext}` (0 hotlinks left, syntax OK), files verified as real image bytes. Fixed `md1` (server returned JPEG under a `.webp` URL → renamed `.jpg`; downloader now sniffs magic bytes so it won't recur). yg1's GOAT image 404'd on the checker's HEAD but downloaded fine on GET (37 KB — smallest, possibly lower-res; swap if it looks weak). REMAINING: (a) eyeball the cards in-browser + commit `item-photos/` + `data.js`; (b) optionally source a crisper yg1 image. NOTE: `images:[]` arrays are empty today, so this pass captures the single hero each item has; **capturing MULTIPLE angles needs the extra URLs sourced first** (per-listing gallery step — Shopify stores expose `{product-url}.json` with the full image list; Etsy/SSENSE/GOAT harder). The downloader already handles extras once the URLs are added.
  - **UPDATE 2026-08-07 (the real coverage):** the item `img:` fields were only ~24 of the images — the **`IMAGES{name:url}` map** (95 entries) is the primary per-item source (`pieceImg` reads `IMAGES[name]` before `item.img`). Extended both tools to parse it; localized **91 of 93** map images (+ the 19 heroes = **111 local total**, `item-photos/{slug}.ext`). Tunic (ZARA z1) Poshmark img 404'd → swapped to a live WooCommerce Zara-tunic photo + localized (approximate stand-in, swap for a real photo anytime). Commits 616ab52 + 33011a6. **ONLY 1 hotlink left:** Roman Fabric Boots (SASAIA, s1) — farfetch **429 rate-limit** (image is fine, still displays in-app); just re-run `python3 tools/localize-images.py` later (rate limit resets) to finish. That's the whole closet de-hotlinked.
  - _original plan:_ Capture MULTIPLE images per product when the listing offers them — one download pass, so grab every useful angle before the listing dies. Model: keep `img` as the hero (all renderers/`pieceImg` read it unchanged), add an optional `images: [...]` array of local paths for extras; single-image items omit it (backward-compatible). Naming: `{id}.jpg` hero, `{id}-2.jpg`/`{id}-3.jpg`… extras. Keep hero + up to ~3 (front/back/detail), skip near-dupes + lifestyle clutter. Storing the extras is independent of the gallery UI (item-popup thumbnail strip using the existing `.item-detail-photo` slot) — do the capture first, wire the gallery separately so durability isn't blocked on UI.
- [~] **On-demand link checker + stale-link handling.** `tools/check-links.mjs` (DONE 2026-08-07) — loads `data.js` via a sandboxed vm, collects every product `url` + hotlinked `img` across `WARDROBE_DATA` + `SHOP` (85 unique links; 19 hotlinked imgs; 5 already local), HEAD-requests each (GET fallback on 403/405/501, redirect-follow, timeout, concurrency), classifies ok/redirect/dead/unreachable, writes `tools/link-report.{md,json}`. Node 18+, no deps. **Must run locally** — the Cowork sandbox proxy blocks arbitrary domains (`EAI_AGAIN`), so egress happens on your machine: `node tools/check-links.mjs` (`--only=img|url`, `--dry-run`, `--concurrency=`, `--timeout=`). Reports are gitignored. REMAINING: (a) run it locally + triage results; (b) wire the app to grey/disable the "View" button where a `url` is stale. For OWNED items a dead URL doesn't change ownership — the item keeps its stats/history; just mark the reference link stale (no lifecycle field). NOTE: a lifecycle status (`active`/`sold-out`/`dead`) only makes sense for SHOP items (purchasable) — track separately if/when useful, not on wardrobe items.
